...
-- @param ply Player
-- @return boolean
PlayX.IsPermitted = function (ply)
     -- Default is Deny
    local result = false
...
PlayX.ResolveProvider = function (provider, uri, useJW)
    local result = nil
...
-- Add Play to History
PlayX.AddToHistory = function (ply, title, url)
	-- Add History to Server
   	table.insert(PlayX.History, {Player = ply:GetName(), Time = os.date("%I:%M:%S %p %y-%m-%d"), Title = title, URL = url})	
...
PlayX.OpenMedia = function (provider, uri, start, forceLowFramerate, useJW, ignoreLength)
    ...
    if result.MetadataFunc then
        ...
        result.MetadataFunc(function(data)
            ...
            -- Add Play to History
            PlayX.AddToHistory(PlayX.PlayerManager, data.Title, data.URL)
        end, function(err)
            Msg(Format("PlayX: Metadata failed via provider %s: %s\n", provider, err))
            -- Add Play to History
            PlayX.AddToHistory(PlayX.PlayerManager, "", uri)
        end)
    end
...
PlayX.UpdateMetadata = function (data)
...
PlayX.BeginMedia = function (handler, uri, start, resumeSupported, lowFramerate, handlerArgs)
    timer.Stop("PlayXMediaExpire")
...
PlayX.SendBeginDStream = function (ply)
    local filter = nil
...
--- Send the PlayXEnd umsg to clients. You should not have much of a
-- a reason to call this method.
PlayX.SendError = function (ply, err)
...
PlayX.SendSpawnDialogUMsg = function (ply, forRepeater)
...
PlayX.Use = function (ply)
    umsg.Start("PlayXUse", ply)
    umsg.End()
end

-- Save History
PlayX.SaveHistory = function ()
	-- Create Dir
    file.CreateDir("playx")
    -- Serialize and Save
    file.Write("playx/history.txt", von.serialize(PlayX.History))
end

-- Read History
PlayX.ReadHistory = function ()
	if file.Exists("playx/history.txt", "DATA") then
	    -- Serialize and Save
	     PlayX.History = von.deserialize(file.Read("playx/history.txt", "DATA"))
    else
    	-- Reset History File
    	PlayX.SaveHistory()
    end
...
local function ConCmdOpen(ply, cmd, args)
    ...
    elseif GetConVar("playx_race_protection"):GetFloat() > 0 and 
        ...
        elseif start < 0 then
            ...
            local result, err = PlayX.OpenMedia(provider, uri, start,  forceLowFramerate, useJW, ignoreLength)
...
local function DSHistoryClear (len, ply)
...
-- Receive PlayX History
net.Receive("PlayXHistory", DSHistoryClear)
