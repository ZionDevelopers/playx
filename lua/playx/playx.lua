...
CreateConVar("playx_shoutcast_host_url", "https://playx.juliocesar.me/shoutcast.host.html",        {FCVAR_GAMEDLL})
...
for _, file in pairs(p) do
    ...
    if not status then
        ErrorNoHalt(PlayX.Translation.get("error_loading_provider", file, err))
    end
end
...
function PlayX.SpawnForPlayer(ply, model, repeater)
    if not repeater and PlayX.PlayerExists() then
        return false, PlayX.Translation.get("error_player_already_exists"), nil
    end
    
    if not util.IsValidModel(model) then
        return false, PlayX.Translation.get("error_invalid_model"), nil
    end
...
function PlayX.ResolveProvider(provider, uri, useJW)
    ...
    if provider ~= "" then -- Provider detected
        if not list.Get("PlayXProviders")[provider] then
            return nil, PlayX.Translation.get("error_unknown_provider")
        end
        ...
        if not result then
            return nil, PlayX.Translation.get("error_no_provider_detected")
        end
    end
...
function PlayX.OpenMedia(provider, uri, start, forceLowFramerate, ignoreLength)
    if not PlayX.PlayerExists() then
        return false, PlayX.Translation.get("error_no_player_spawned")
    end
    ...
    if result.MetadataFunc then
        Msg(PlayX.Translation.get("metadata_function_available", provider))
        result.MetadataFunc(function(data)
            Msg(PlayX.Translation.get("metadata_received_via_provider", provider))
            PlayX.UpdateMetadata(data)
        end, function(err)
            Msg(PlayX.Translation.get("metadata_failed", provider, err))
        end)
    end
...
function PlayX.UpdateMetadata(data)
    ...
    if data.Length then
	    if GetConVar("playx_expire"):GetFloat() <= -1 then
	        ...
	        print(PlayX.Translation.get("time_until_expire", length, timeLeft))
	        
	        if timeLeft > 0 then
	            timer.Adjust("PlayXMediaExpire", timeLeft, 1)
	            timer.Start("PlayXMediaExpire")
	        else -- Looks like it ended already!
	            print(PlayX.Translation.get("media_expired"))
	            PlayX.EndMedia()
	        end
	    end
    end
...
function PlayX.SendSpawnDialogUMsg(ply, forRepeater)
    ...
    elseif not PlayX.IsPermitted(ply) then
        PlayX.SendError(ply, PlayX.Translation.get("not_permitted_to_use"))
    else
...
local function ConCmdOpen(ply, cmd, args)
    ...
    elseif not PlayX.PlayerExists() then
        PlayX.SendError(ply, PlayX.Translation.get("error_no_player_spawned"))
    elseif not args[1] then
        ply:PrintMessage(HUD_PRINTCONSOLE, PlayX.Translation.get("error_no_uri_cmd"))
    elseif GetConVar("playx_race_protection"):GetFloat() > 0 and 
        ...
        PlayX.SendError(ply, PlayX.Translation.get("error_race_protection"))
    else
        ...
        elseif start < 0 then
            PlayX.SendError(ply, PlayX.Translation.get("error_negative_start_time"))
        else
            PrintMessage(HUD_PRINTCONSOLE, PlayX.Translation.get("video_started", ply:Nick()))
...
function ConCmdSpawn(ply, cmd, args)
    ...
    elseif not PlayX.IsPermitted(ply) then
        ...
        if not args[1] or args[1]:Trim() == "" then
            PlayX.SendError(ply, PlayX.Translation.get("error_no_model"))
        else
...
function PlayerAuthed(ply, steamID, uniqueID)
    if PlayX.CurrentMedia and PlayX.AdminTimeoutTimerRunning then
        if PlayX.IsPermitted(ply) then
            print(PlayX.Translation.get("admin_joined_during_timeout"))
...
function PlayerDisconnected(ply)
    ...
    if timeout > 0 then
        print(PlayX.Translation.get("admin_timeout_set", timeout))
...
timer.Adjust("PlayXAdminTimeout", 1, 1, function()
    print(PlayX.Translation.get("admin_timeout_ended"))
            ...
            if searchTable.items then
                if searchTable.items[1] then
                    if searchTable.items[1].id then
                        ...
                        failureF(PlayX.Translation.get("error_occurred_while_querying_youtube"))
                    end
                else
                    failureF(PlayX.Translation.get("error_occurred_while_querying_youtube"))
                end
