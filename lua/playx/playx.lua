...
for _, file in pairs(p) do
    ...
    if not status then
        ErrorNoHalt(PlayX.translate("error_loading_provider", file, err))
    end
end
...
function PlayX.SpawnForPlayer(ply, model, repeater)
    if not repeater and PlayX.PlayerExists() then
        return false, PlayX.translate("error_player_already_exists"), nil
    end
    
    if not util.IsValidModel(model) then
        return false, PlayX.translate("error_invalid_model"), nil
    end
...
function PlayX.ResolveProvider(provider, uri, useJW)
    ...
    if provider ~= "" then -- Provider detected
        if not list.Get("PlayXProviders")[provider] then
            return nil, PlayX.translate("error_unknown_provider")
        end
        ...
        if not result then
            return nil, PlayX.translate("error_provider_not_recognize", uri)
        end
        ...
        if not result then
            return nil, PlayX.translate("error_no_provider_detected")
        end
    end
...
function PlayX.OpenMedia(provider, uri, start, forceLowFramerate, useJW, ignoreLength)
    if not PlayX.PlayerExists() then
        return false, PlayX.translate("error_no_player_spawned")
    end
    ...
    if uri == "" then
        return false, PlayX.translate("error_no_uri")
    end
    ...
    if result.MetadataFunc then
        Msg(PlayX.translate("metadata_function_available", provider))
        result.MetadataFunc(function(data)
            Msg(PlayX.translate("metadata_received_via_provider", provider))
            PlayX.UpdateMetadata(data)
        end, function(err)
            Msg(PlayX.translate("metadata_failed", provider, err))
        end)
    end
...
function PlayX.SendSpawnDialogUMsg(ply, forRepeater)
    ...
    elseif not PlayX.IsPermitted(ply) then
        PlayX.SendError(ply, PlayX.translate("not_permitted_to_use"))
    else
...
local function ConCmdOpen(ply, cmd, args)
    ...
    elseif not PlayX.PlayerExists() then
        PlayX.SendError(ply, PlayX.translate("error_no_player_spawned"))
    elseif not args[1] then
        ply:PrintMessage(HUD_PRINTCONSOLE, PlayX.translate("error_no_uri_cmd"))
    elseif GetConVar("playx_race_protection"):GetFloat() > 0 and 
        ...
        PlayX.SendError(ply, PlayX.translate("error_race_protection"))
    else
        ...
        if start == nil then
            PlayX.SendError(ply, PlayX.translate("error_invalid_start_time"))
        elseif start < 0 then
            PlayX.SendError(ply, PlayX.translate("error_negative_start_time"))
        else
            PrintMessage(HUD_PRINTCONSOLE, PlayX.translate("video_started", ply:Nick()))
...
function ConCmdSpawn(ply, cmd, args)
    ...
    elseif not PlayX.IsPermitted(ply) then
        ...
        if not args[1] or args[1]:Trim() == "" then
            PlayX.SendError(ply, PlayX.translate("error_no_model"))
        else
...
function PlayerAuthed(ply, steamID, uniqueID)
    if PlayX.CurrentMedia and PlayX.AdminTimeoutTimerRunning then
        if PlayX.IsPermitted(ply) then
            print(PlayX.translate("admin_joined_during_timeout"))
...
function PlayerDisconnected(ply)
    ...
    if timeout > 0 then
        print(PlayX.translate("admin_timeout_set", timeout))
...
timer.Adjust("PlayXAdminTimeout", 1, 1, function()
    print(PlayX.translate("admin_timeout_ended"))
            ...
            if searchTable.items then
                if searchTable.items[1] then
                    if searchTable.items[1].id then
                        ...
                        failureF(PlayX.translate("error_occurred_while_querying_youtube"))
                    end
                else
                    failureF(PlayX.translate("error_occurred_while_querying_youtube"))
                end
