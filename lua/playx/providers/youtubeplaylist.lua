    ...
    if uri[1]:find("^[A-Za-z0-9_%-]+$") and uri[2]:find("^[A-Za-z0-9_%-]+$") then
        ...
        return {
            ...
            ["MetadataFunc"] = function(callback, failCallback)
                YouTubePlaylist.QueryMetadata(uri, callback, failCallback)
            end
        }
    end
end

function YouTubePlaylist.QueryMetadata(uri, callback, failCallback)
  local vars = {
  ...
  local function successF (resultsTable)        
        ...
        if length then
            callback({
                ["URL"] = "https://www.youtube.com/watch?v=" .. uri[1],
                ["Title"] = title,
                ["Description"] = desc,
                ["Length"] = length,
                ["Tags"] = tags,
...
list.Set("PlayXProviders", "YouTubePlaylist", YouTubePlaylist)
list.Set("PlayXProvidersList", "YouTubePlaylist", {"YouTube Playlist"})
